...
def braak_parallel(params_):
    ...
    for ii, set_ in enumerate(params_):
        ...
        maxHe, mCie, mCee, maxTAU2SC, rho, HAdamrate, cABinh, cTAUinh = 0.35, 20.5, 80, 0.3, 50, 5, 0.4, 1.8
        ...
        posterior_rois = ['ctx-lh-cuneus', 'ctx-lh-inferiorparietal', 'ctx-lh-isthmuscingulate',
                          ...
                          'ctx-lh-lingual', 'ctx-lh-pericalcarine',  'ctx-lh-posteriorcingulate',
                          ...
                          'ctx-rh-lateraloccipital',
                          'ctx-rh-lingual', 'ctx-rh-pericalcarine',  'ctx-rh-posteriorcingulate',
        ...
        if useCB:
            ...
            bi_pos = list(set([roi[7:] for roi in posterior_rois if roi[:3] == "ctx"]))
            bi_ant = list(set([roi[7:] for roi in anterior_rois if roi[:3] == "ctx"]))

            if mode == "fixed":
                ##  REGIONAL SEEDs for toxic proteins
                AB_seeds = ["ctx-lh-precuneus", "ctx-lh-isthmuscingulate", "ctx-lh-insula",
                            "ctx-lh-medialorbitofrontal", "ctx-lh-lateralorbitofrontal", "ctx-lh-posteriorcingulate",
                            "ctx-rh-precuneus", "ctx-rh-isthmuscingulate", "ctx-rh-insula",
                            "ctx-rh-medialorbitofrontal", "ctx-rh-lateralorbitofrontal", "ctx-rh-posteriorcingulate"]
                ...
                # Randomize the pairs of regions selected, keeping the same max global number - AB 6 pairs; tau 1 pair.
                random_seeds = np.random.randint(0, len(bilateral_rois), size=6)
                ...
                # Randomize the pairs of regions selected, keeping the same max global number - AB 6 pairs; tau 1 pair.
                random_seeds = np.random.randint(0, len(bi_pos), size=3)
                AB_seeds = ["ctx-lh-" + bi_pos[id] for id in random_seeds] +\
                           ["ctx-rh-" + bi_pos[id] for id in random_seeds]
                ...
                # Randomize the pairs of regions selected, keeping the same max global number - AB 6 pairs; tau 1 pair.
                random_seeds = np.random.randint(0, len(bi_ant), size=2)
                AB_seeds = ["ctx-lh-" + bi_ant[id] for id in random_seeds] + \
                           ["ctx-rh-" + bi_ant[id] for id in random_seeds]
                ...
                # Randomize the pairs of regions selected, keeping the same global number - AB 6 pairs; tau 1 pair.
